#AA2 for TST by Y. Zuo on 01/08/22
library(mvtnorm)
library(robustbase)
library(compiler)
enableJIT(1)
# will call functions: get_initial_beta to get initial beta^0, get_weight_wi for the weights
# they are defined in AA1-1.R or AA1.R or AA11.R
...
{ #z is a data matrix n by p, in the ith row, the first (p-1) columns for x_i  and pth column for y_i 
  #w_i is a 1 by n row vector, obtained by get_weight_wi function
  ...
  objective_value=t(rn)%*%wnn%*%rn           #Q(beta_k)
...
{ #z is a data matrix n by p, in the ith row, the first (p-1) columns for x_i  and pth column for y_i 
  ...
  nabla_f_beta_k=-2*t(xnn)%*%wnn%*%rn        #p by 1 vector-the gradient at beta_k
                                             #here we have used the 1st-order derivation of Q(beta)
...
{ #modified_hessian is the matrix H(x)+gamma I
  #nabla_f_beta_k is the gradient of Q at beta_k
...
{ #z is a data matrix n by p, in the ith row, the first (p-1) columns for x_i  and pth column for y_i 
  ...
  Delta_x=s_k; alpha1=1/4; beta1=1/2; t=1
...
AA2_main=function(z, alpha, choice_value_4_initial_beta, gamma, epsilon) 
{#z is a data matrix n by p, in the ith row, the first (p-1) columns for x_i  and pth column for y_i 
 ...
 beta_k=get_initial_beta(z,c)
 wi=get_weight_wi(z,beta_k,alpha)
 ...
 {                                    #while((norm_gradient>=epsilon)&(k<50))
   ...
   t=get_modified_Hessian_matrix(z, gamma, wi)
                                 #check if it is invertible, then a_k=-(hessian)^{-1}nabla_f_beta_k
                                 #so that to avoid call search_a_k to get a_k
   ...
   { 
    ...
    k=k+1                                            # k actually is useless, 
                                                     # beta_k could be replaced by beta_new and
                                                     # beta_old, to avoid k
    wi=get_weight_wi(z,beta_k,alpha)
    nabla_f_beta_k=get_gradient_at_beta_k(z, beta_k, wi)
    norm_gradient=norm(nabla_f_beta_k, type="2")
